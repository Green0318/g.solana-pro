...
const main = async () => {
  ...
  const my_tracks = await program.account.track.all([{memcmp:{offset:8, bytes: signer.publicKey.toBase58()}}])
  for (const track of  my_tracks) {
    console.log(`Artist: ${track.account.artist}, Title: ${track.account.trackTitle}, cid: ${track.account.cid}`)
